<script lang="ts">
    ...
    const fetchData = async (typeParam: string, keyParam: string) => {
        ...
        const topicStartKey = encodeURIComponent(
            JSON.stringify(
                keyParam
                    ? [typeParam, desc ? 0 : 0, restKey]
                    : [typeParam, desc ? 1 : 0]
            )
        );
        const topicEndKey = encodeURIComponent(
            JSON.stringify([typeParam, desc ? -1 : 1])
        );
        const keys =
            type === "all"
                ? keyParam
                    ? `&startkey=${allStartKey}`
                    : ""
                : `&startkey=${topicStartKey}&endkey=${topicEndKey}`;
        const fetchUrl = `${baseUrl}${
            type === "all" ? "byDateTime" : "byThemeAndDateTime"
        }?descending=${desc}&limit=${desc ? pageSize : pageSize + 1}${keys}`;
        ...
        if (fetchedArticles.rows.length == pageSize) {
            nextKey =
                (typeParam === "all"
                    ? fetchedArticles.rows.pop()?.key
                    : fetchedArticles.rows.pop()?.key[2]) || "";
        }

        if (fetchedArticles.rows.length > 0 && !firstPage) {
            prevKey =
                typeParam === "all"
                    ? fetchedArticles.rows[0].key
                    : fetchedArticles.rows[0].key[2];
        }
